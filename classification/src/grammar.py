...
class GrammarRules:
    def __init__(self):
        ...
        self.verbs.extend(['submit', 'check', 'start', 'fill', 'process', 'grade',
                           ...
                           'expected', 'make', 'made', 'makes', 'beginning'])
...
class GrammarTools:
    ...
    @staticmethod
    def extract_object(words, object):
        generalized_words = []
        object_words = GrammarTools.str2words(object)
        i = 0
        while i < len(words):
            match = False
            if words[i] in object_words:
                # Check that the all the object_words are continuously in words
                j = i
                match = True
                for k in range(len(object_words)):
                    if not words[j] == object_words[k]:
                        match = False
                        break
                    j += 1

                # If a match was found, skip to the word after the object
                if match:
                    i = j

            # Extract the object words if they match, otherwise continue on
            if match:
                to_append = '<object>'
            else:
                to_append = words[i]
                i += 1
            generalized_words.append(to_append)
